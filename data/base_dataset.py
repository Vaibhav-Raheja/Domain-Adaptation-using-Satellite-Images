...
def get_transform(opt, params=None, grayscale=False, method=Image.BICUBIC, convert=True):
    ...
    if "r1" in opt.preprocess:
        transform_list.append(transforms.Lambda(lambda img: __resize_to_thresh(img, opt.short_side_thresh,
                                                                               opt.long_side_thresh)))
...
def __lighting_augment(img):
    pass

def __resize_to_thresh(img, short_side_thresh=512, long_side_thresh=768, method=Image.BICUBIC):
    '''
    new_h = oh//2
    new_w = ow//2
    ...
    shortside = min(ow, oh)
    longside = max(ow,oh)
    #No need to resize if size is less than target_width
    if shortside <= short_side_thresh and longside<=long_side_thresh:
        return img
    #If short_side>512, or longside>768, resize it
    else:
        if longside > long_side_thresh:
            scale = longside /long_side_thresh
        else:
            scale = shortside / short_side_thresh
