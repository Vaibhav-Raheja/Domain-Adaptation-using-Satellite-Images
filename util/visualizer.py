...
def save_images(webpage, visuals, image_path, save_image_prefix_len, aspect_ratio=1.0, width=256):
    ...
    dirname = ntpath.dirname(image_path[0])
    split_list = dirname.rsplit("/", save_image_prefix_len)
    save_prefix = seperator.join(split_list[-save_image_prefix_len:])
    ...
    for label, im_data in visuals.items():
        ...
        image_name = '%s/%s/%s.tif' % (label, save_prefix, name)
        os.makedirs(os.path.join(image_dir, label, save_prefix), exist_ok=True)
        save_path = os.path.join(image_dir, name+".tif")
